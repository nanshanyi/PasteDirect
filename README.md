...
自签名证书系统会有安全提示，系统设置->隐私与安全性->仍然打开即可

### 更新无忧

从 v2.2.0 起，应用采用自签名证书机制，更新时辅助功能权限自动保留，无需重新授权。

从 v3.6.0 起，应用内置 [Sparkle](https://sparkle-project.org/) 自动更新：检测到新版本后可直接在应用内下载并自动安装重启，无需再手动下载 dmg；也可以在设置中开启「自动下载更新」，静默下载、退出时自动安装。
...
| `Space` | 预览选中项 |
| `Delete` | 删除选中项 |
| `Esc` | 依次关闭预览 / 筛选弹窗 / 清空搜索筛选 / 关闭面板 |
| 拖拽 | 将条目拖拽到目标位置直接粘贴 |
| 输入文字 | 搜索过滤历史记录，`Backspace` 可逐个移除筛选条件 |
...
- **忽略应用** — 配置不记录剪贴板的应用列表
- **快捷键** — 自定义全局呼出快捷键
- **自动更新** — 基于 Sparkle：自动检查新版本、应用内下载、自动安装重启；可选「自动下载更新」静默更新
